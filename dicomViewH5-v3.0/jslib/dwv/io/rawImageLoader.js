...
dwv.io.RawImageLoader = function ()
{
    ...
    this.load = function ( dataUri, origin, index ) {
        // create a DOM image
        var image = new Image();
        // dataUri = origin = "http://ovhjo13ga.bkt.clouddn.com/734214185114566656-4-30.jpg"
            ...
            try {
                taskTime = setInterval(function(){
                    if(!taskFlag){
                        taskFlag = true;
                        setTimeout(function(){
                            dwv.image.getViewFromDOMImage(taskList[taskCount],function(data,img){
                                self.onload(data,img);
                                self.onloadend();
                                taskFlag = false;
                            },true);
                            taskCount ++;
