...
dwv.App = function ()
{
    ...
    var picquality = "";
    var slice3DFn = null;
    this.setSlice3DFn = function(value){slice3DFn = value;}
    this.getSlice3DFn = function(){return slice3DFn;}
    this.getIs3D= function(){return is3D;}
    this.setIs3D = function(value){is3D = value}
    this.getThreeDDataHeight= function(){return threeDDataHeight;}
    this.setThreeDDataHeight = function(value){threeDDataHeight = value}
    this.getLoadCount = function(){return loadCount;}
    this.setPicQuality = function(value){picquality = value}
    this.getPicQuality = function(){return picquality}
    this.getInstanceNum = function(){return instanceNum;}
    this.setInstanceNum = function(value){instanceNum = value;}
    this.setFitToWindow = function(value){
        fitToWindow = value;
        var size = this.getImage().getGeometry().getSize();
    ...
    this.setSeries = function(type){series = type;}
    // allscroll
    this.getAllScroll = function(){return allscroll}
    this.setAllScroll = function(type){allscroll = type;}

    this.getAppData = function(){return appData;}
    /**
    ...
    this.init = function ( config ) {
        ...
        is3D = config.is3D;
        threeDDataHeight = config.threeDDataHeight;
        // tools
    ...
    this.resetLayout = function () {
        ...
        if ( imageLayer ) {
            ...
            var originalImage = view.getImage();
            // layout
            var size = originalImage.getGeometry().getSize();
            dataWidth = size.getNumberOfColumns();
            dataHeight = size.getNumberOfRows();
            // if(originalImage.getIsRotate())
            //     offsetDraw.y = -dataHeight;
            if(self.getIs3D()){
                if(self.getImage().getIsRotate()){
                    var temp = dataWidth;
                    dataWidth = dataHeight;
                    dataHeight = temp;
                }
                dataHeight = self.getThreeDDataHeight();
                imageLayer.draw(offsetDraw.x,offsetDraw.y,originalImage.getIsRotate(),dataWidth,dataHeight);
            }else{
                imageLayer.draw(offsetDraw.x,offsetDraw.y,originalImage.getIsRotate());
            }
        }
        if ( drawController ) {
    ...
    function loadImageData(data, loader, options)
    {
        ...
        loader.onload = function(data,img){
            if(!data) return;
            loadCount++;
            try {
                if ( image ) {
                    view.append( data.view,img );
                    if ( drawController ) {
        ...
        loader.onprogress = function(e){
            onLoadProgress.call(this,e)
        };
        // main load (asynchronous)
        fireEvent({ 'type': 'load-start' });
        if(image){
            loader.load(data, options,image.getNumberOfFrames());
        }else{
            loader.load(data, options);
        }
    }
    this.appendCalBuffer = function(bview,buffer){
        if ( image ) {
            view.append( bview,null,buffer);
            if ( drawController ) {
                drawController.appendDrawLayer(image.getNumberOfFrames());
            }
        }else{
     ...
     */
    this.fitToSize = function (size,flag)
    {
        if(this.getIs3D()){
            if(self.getImage().getIsRotate()){
                var temp = dataWidth;
            ...
            dataHeight = this.getThreeDDataHeight();
        }
        // dataHeight = 512;
        // previous width
        ...
        if(newWidth < container.clientWidth){
            offsetDraw.x = (container.clientWidth - newWidth)/(2*windowScale);
            if(offsetDraw.x < 0) offsetDraw.x = 0;
        }else{
            offsetDraw.x = 0;
        }
        if(newHeight < container.clientHeight){
            offsetDraw.y = (container.clientHeight - newHeight)/(2*windowScale);
            if(offsetDraw.y < 0) offsetDraw.y = 0;
        }else{
            offsetDraw.y = 0;
        }
        // newWidth = container.clientWidth
        ...
        // resize image layer
        if ( imageLayer ) {
            imageLayer.setWidth(container.clientWidth);
            imageLayer.setHeight(container.clientHeight);
            ...
            // dataHeight = size.getNumberOfRows();
            if(originalImage.getIsRotate()){
                var tempY = offsetDraw.y;
                offsetDraw.y = -dataHeight-offsetDraw.x;
                offsetDraw.x = tempY;
            }
        ...
        // resize draw stage
        if ( drawController ) {
            drawController.resizeStage(container.clientWidth, container.clientHeight, scale);
        }
    };
    ...
    */
        ...
        if(fitToWindow){
            if(self.getImage().getIsRotate()){
                self.fitToSize( {
                    ...
                    // 'height': self.getElement("layerContainer").offsetWidth,
                    // 'width': self.getElement("layerContainer").offsetHeight 
                } );
            }else{
                self.fitToSize( {
                    'width': self.getElement("layerContainer").offsetWidth,
     ...
     * @param {Object} event The change event.
     */
    this.onChangeTool = function (/*event*/value)
    {
        var value2 = typeof value == "string"?value:this.value
        // called from an HTML select, use its value
        toolboxController.setSelectedTool(value2);
    };
    ...
    this.generateAndDrawImage = function(flag){
        generateAndDrawImage(flag);
    }
    /**
     * Generate the image data and draw it.
     */
    function generateAndDrawImage(flag)
    {
        var originalImage = view.getImage();
        // layout
        var size = originalImage.getGeometry().getSize();
        ...
        view.generateImageData(imageData,flag);
        // set the image data of the layer
        imageLayer.setImageData(imageData,originalImage.getIsRotate());
        // draw the image
    ...
    function createLayers(dataWidth, dataHeight)
    {
        ...
        if ( drawDiv ) {
            ...
            drawController.create(self.getElement("layerContainer").offsetWidth, self.getElement("layerContainer").offsetHeight);
        }
        // resize app
        if ( fitToWindow ) {
            // self.fitToSize( dwv.gui.getWindowSize() );
    ...
    this.postLoadInit=function(data,flag){
        postLoadInit(data,flag);
    }
    /**
     ...
     */
    function postLoadInit(data,flag)
    {
        ...
        if ( drawController ) {
            if(flag){
                drawController.appendDrawLayer(instanceNum);
                drawController.activateDrawLayer(viewController);
            }else{
                drawController.appendDrawLayer(image.getNumberOfFrames());
            }
