...
dwv.ViewController = function ( view )
{
    ...
    var playerID = null;
    var fpsRefreshID = null;
    //playspeed
    var playspeed = 50;
    var fps = 0;
    var fpsSpeed = 0;
    this.getJpgReversalFlag = function(){
        return view.getJpgReversalFlag();
    }
    this.getFpsSpeed = function(){
        return fpsSpeed;
    }
    this.setFpsSpeed = function(value){
        return fpsSpeed = value;
    }
    ...
    {
        //循环查看
        var currentSlice = view.getCurrentPosition().k + 1;
        if(currentSlice>=view.getImage().getGeometry().getSize().getNumberOfSlices()){
            currentSlice = 0
        }
        return self.setCurrentSlice( currentSlice );
    };
    ...
    this.decrementSliceNb = function ()
    {
        ...
        var currentSlice = view.getCurrentPosition().k - 1;
        if(currentSlice<0){
            currentSlice = view.getImage().getGeometry().getSize().getNumberOfSlices()-1
        }
        return self.setCurrentSlice( currentSlice );
    };
     ...
     */
     this.play = function ()
     {
         if ( playerID === null ) {
             ...
             fps = 0;
            fpsRefreshID = setInterval(function(){
                self.setFpsSpeed(self.getPlaySpeed());
            },1000);
      ...
      */
      this.stop = function ()
      {
          ...
          if( fpsRefreshID !== null){
            clearInterval(fpsRefreshID);
            fpsRefreshID = null;
          }
      };
