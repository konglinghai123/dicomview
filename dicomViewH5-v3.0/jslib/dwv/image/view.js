...
dwv.image.View = function (image)
{
    ...
    var jpgReversalFlag = false;
    
    this.getJpgReversalFlag = function() { return jpgReversalFlag; };
    
    this.setJpgReversalFlag = function(flag) { jpgReversalFlag = flag; };
    /**
     ...
     */
    this.append = function( rhs,pic,buffer,get3DFn )
    {
       // append images
       var newSliceNumber = this.getImage().appendSlice( rhs.getImage(),pic,buffer,get3DFn );
    ...
    this.clone = function ()
    {
        ...
        //删除，避免事件绑定到之前view上，需重新再在app中绑定事件
        // var listener = new Object();
        // Object.assign(listener,this.getListeners());
        // copy.setListeners(listener);
        return copy;
    };
 ...
 */
dwv.image.View.prototype.generateImageData = function( array , flag)
{
    ...
    case "RGB":
        ...
        if(flag === 1){//反片切换
            this.setJpgReversalFlag(!this.getJpgReversalFlag())
        }else if(flag === 2){
            this.setJpgReversalFlag(false)
        }//重置反片
        posR = 0;
        posG = 1;
        posB = 2;
        stepPos = 4;
        var putArray = image.getValueAtOffset(sliceIndex);
        for(var j=0; j < sliceSize; ++j)
        {
            ...
            array.data[index+2] = parseInt( windowLut.getValue(
                putArray[posB] ), 10 );
            if(this.getJpgReversalFlag()){
                array.data[index] = 255 - array.data[index];
                array.data[index+1] = 255 - array.data[index+1];
                array.data[index+2] = 255 - array.data[index+2];
            }
            array.data[index+3] = 0xff;
            index += 4;
...
dwv.image.ViewFactory.prototype.create = function (dicomElements, image)
{
    ...
    if ( typeof dwv.tool.defaultpresets !== "undefined" ) {
        var modality = "CT"||image.getMeta().Modality;
