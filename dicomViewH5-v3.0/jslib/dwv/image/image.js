 ...
 */
dwv.image.Image = function(geometry, buffers, numberOfFrames, imgPics,isrotate,get3DFn)
{
    ...
    var get3DFn = get3DFn;
    this.get3DFn = function(){
        return get3DFn;
    }
    var isrotate = isrotate;
    this.getIsRotate = function(){ 
        return isrotate;
    }
    /**
     * Rescale slope and intercept.
    ...
    var numberOfComponents = imgPics?imgPics.length:buffers.length;
    /**
     * Meta information.
     * @private
     * @type Object
     ...
     */
    this.setFirstOverlay = function (over,flag) { 
        overlays[0] = over; 
        if(flag) overlays = over;
    };
    ...
    this.getPics = function(){ return imgPics;}
    /**
     * Get the data buffer of the image.
     * @todo dangerous...
     * @return {Array} The data buffer of the frame.
     */
    this.getFrame = function (frame) { return buffers[frame]; };
     ...
     */
    this.getNumberOfComponents = function() { return imgPics?imgPics.length:buffers.length; };
     ...
     */
    this.getValueAtOffset = function (index) {
        var idata = null;
        if(buffers && buffers.length>0){
            idata = buffers[index];
            if(!(idata instanceof Uint8Array)){
                idata = this.get3DFn()(index).buffer;
            }
        }else if(imgPics && imgPics.length>0){
            var image = imgPics[index];
            ...
            idata = new Uint8ClampedArray(imageData.data.buffer);
        }
        return idata;
    };
    ...
    this.clone = function()
    {
        ...
        var copy = new dwv.image.Image(this.getGeometry(),imgPics);
        // copy the RSIs
     ...
     */
    this.appendSlice = function (rhs,pic, buffer)
    {
        // check input
        if( rhs === null ) {
        ...
        if(pic){
            ...
            {
                tempArray.push(pic);
                tempArray = tempArray.concat(imgPics);
            }
            ...
            else
            {
                ...
                tempArray = tempArray.concat(imgPics.slice(0,newSliceNb))
                tempArray.push(pic);
                tempArray = tempArray.concat(imgPics.slice(newSliceNb))
            }
            imgPics = tempArray;
        }else if(buffer){
            if( newSliceNb === 0 )
            {
                ...
                // tempArray = tempArray.concat(buffers);
            }
            else if( newSliceNb === size.getNumberOfSlices() )
            {
                tempArray = tempArray.concat(buffers);
            ...
            {
                var offset = newSliceNb * sliceSize;
                tempArray = tempArray.concat(buffers.slice(0,newSliceNb))
                tempArray.push(buffer);
                tempArray = tempArray.concat(buffers.slice(newSliceNb))
            }
            buffers = tempArray;
        }
        
        // update geometry
        if( newSliceNb === 0 && buffer){
            size.setNumberOfSlices(1);
        }else{
...
dwv.image.Image.prototype.getValue = function( i, j, k, f )
{
    ...
    return this.getValueAtOffset( k );
};
...
dwv.image.Image.prototype.calculateDataRange = function ()
{
    ...
    var min = 0;
    // var max = this.getValueAtOffset(0)[0];
    var max = 255;
    var value = 0;
